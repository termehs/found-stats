...
While the expected value tells us the average, the variance of a random variable, denoted $\text{Var}(X)$, tells us how spread out the values are around the mean. A small variance means values cluster tightly around the mean; a large variance means they are more widely scattered.
