...
#### Example: Standard Normal Distribution {.unnumbered}
...
#### Example: $X \sim \mathcal{N}(170, 10)$ {.unnumbered}
...
As mentioned earlier, one of the greatest strengths of the normal distribution is its flexibility; it can be used to approximate probabilities in many important situations, especially when working with other discrete distributions.
...
# Plot with animation
plot <- ggplot() +
  geom_col(data = binom_all, aes(x = x, y = prob), fill = "gray30", width = 0.9) +
  geom_line(data = normal_all, aes(x = x, y = y), color = "red", linewidth = 1) +
  labs(
    title = "Binomial Distribution Approaching Normal",
...
# Save animation
# anim_save("binomial_to_normal_fixed.gif", animation = plot, fps = 2)
