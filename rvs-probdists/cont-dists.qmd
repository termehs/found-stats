...
Let us visualize this with the previouslt mentioned empirical rule in  @fig-emprule, which summarizes how the area is distributed across standard deviations from the mean. This is shown in @fig-normal-emprule.
...
By using the standard normal, we only need to consult a single probability table, shown in @sec-normaltbl, to find probabilities for a wide range of problems. The transformation is done through **standardization**, which we'll explore in the next section.
