...
::: {.callout-note}
## How Whiskers Are Computed in a Boxplot

In a standard boxplot, whiskers are based on the interquartile range (IQR), not simply the minimum and maximum values.
...
- The **lower whisker** is the smallest value $\geq Q_1 - 1.5 \times IQR$  
- The **upper whisker** is the largest value $\leq Q_3 + 1.5 \times IQR$

Values outside these bounds are shown as **outliers**.

Whiskers are not necessarily symmetric, they reflect the data distribution.
:::
...
# Data
data_A <- c(4, 4, 5, 5, 5, 6, 6, 7)
data_B <- c(0, 1, 4, 5, 6, 7, 8, 11)

# Function to compute boxplot stats using quantile(type = 6)
box_stats <- function(x, group) {
  qs <- quantile(x, probs = c(0.25, 0.5, 0.75), type = 6)
  iqr <- qs[3] - qs[1]
...
box_data <- rbind(
  box_stats(data_A, "Dataset A"),
  box_stats(data_B, "Dataset B")
)

box_data$Group <- factor(box_data$Group, levels = c("Dataset A", "Dataset B"))

ggplot(box_data, aes(x = Group)) +
  geom_boxplot(
    aes(ymin = ymin, lower = lower, middle = middle, upper = upper, ymax = ymax, fill = Group),
    stat = "identity",
    alpha = 0.6
  ) +
  scale_y_continuous(
    breaks = 0:12,        # show 1,2,3,4,5,6,7
    limits = c(0, 12)     # restrict view to this range
  ) +
  labs(x = "Dataset", y = "Score") +
  theme_minimal() +
  theme(legend.position = "none")
```
