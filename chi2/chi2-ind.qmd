...
To explore this further, we can examine the relative percentages across income categories, i.e. the distribution of housing costs within each income category:
...
Note that what we've actually computed above is:
$$
P(\text{Housing Cost} \mid \text{Income})
$$

because each income column sums to 100%.
...
| Housing Cost | Low Income | Medium Income | High Income |
|--------------|-------------|------------|----------------|-------------|
| Low          |  20%        | 20%            | 20%         |
| Medium       |  40%        | 40%            | 40%         |
| High         |  40%        | 40%            | 40%         |

These conditional distributions should all equal the marginal distribution, i.e.
$$ P(\text{Housing Cost}\mid\text{Income}) =P(\text{Housing Cost})
$$

Using expected proportions, the expected counts under $H_0$ computed as
$$E_{ij}=\frac{(\text{Row Total}_i)(\text{Column Total}_j)}{\text{Grand Total}}
$$

which directly come from
$$
P(\text{Housing Cost and Income})
=
P(\text{Housing Cost})P(\text{Income}).
$$
These values are giveen in below table:
