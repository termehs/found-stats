...
One of the key assumptions of the classicxal linear regression model is that the variance of the error term remains constant across all observations. This assumption is known as **homoskedasticity**, and it is formally stated as:
...
$$
H_0: \operatorname{Var}(e_i \mid x_1, x_2, \dots, x_k) = \sigma^2 \quad \text{(homoskedasticity)}
$$

against

$$
H_1: \operatorname{Var}(e_i \mid x_1, x_2, \dots, x_k) \neq \sigma^2 \quad \text{(heteroskedasticity)}
$$
...
Although endogeneity presents a serious challenge for regression analysis, it does not mean that causal effects can never be identified. When suitable sources of exogenous variation are available, alternative estimation methods can recover consistent estimates even when OLS fails. The most widely used of these methods is **instrumental variables (IV) estimation**, which we introduce in the next chapter @sec-iv.
