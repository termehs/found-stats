...
Just as with simple regression, multiple regression relies on several key assumptions:

1. **Linearity**: The relationship between $y$ and the predictors is linear in the parameters.
2. **Fixed predictors**: The $x$-values are treated as fixed (non-random) in repeated samples.
3. **Independent errors**: The errors $\varepsilon_i$ are independent across observations.
4. **Homoskedasticity**: The errors have constant variance: $\operatorname{Var}(\varepsilon_i) = \sigma^2$.
5. **Normality**: The errors are normally distributed: $\varepsilon_i \sim \mathcal{N}(0, \sigma^2)$.

These assumptions are needed to ensure valid estimation, inference, and prediction. We estimate the unknown parameters $\beta_0, \dots, \beta_K$ using the **least squares method**, just as in simple regression. The goal is to minimize the sum of squared differences between observed and predicted values:

$$
\text{SSE} = \sum_{i=1}^n (y_i - \hat{y}_i)^2 = \sum_{i=1}^n (y_i - b_0 - b_1 x_{1i} - \dots - b_K x_{Ki})^2
$$

This yields the **least squares estimates** $b_0, b_1, \dots, b_K$.

Under the model assumptions, these estimates are **unbiased**:

$$
E[b_k] = \beta_k \quad \text{for all } k = 0, \dots, K
$$

This means that, on average, the estimated coefficients correctly reflect the true population parameters.

### Residual Variance and Model Fit

After estimating the regression function, we estimate the variance of the errors using the **residual mean square**:

$$
s_e^2 = \frac{SSE}{n - K - 1} = \frac{\sum (y_i - \hat{y}_i)^2}{n - K - 1}
$$

This estimate tells us how much observed values tend to deviate from their predicted values. It generalizes the residual variance concept from simple regression to cases with multiple predictors.
