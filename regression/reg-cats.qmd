...
ggplot(plot_df, aes(x = exper, y = wage_hat, colour = Education)) +
  geom_line(size = 1.2) +
  labs(
    title = "Interaction of Education and Experience on Wage",
...
# -------------------------------------------------------------------
# 1. Coefficient sets -----------------------------------------------
# (You can replace these with estimates from your own regression.)
beta0 <- 5        # intercept for men
beta1 <- -1       # female dummy (women vs. men, at zero educ)
beta2 <- 0.4      # return to education for men
beta3_neg <- -0.05  # interaction: gap widens (β3 < 0)
beta3_pos <-  0.05  # interaction: gap narrows/reverses (β3 > 0)
...
grid$Gender <- factor(grid$female, labels = c("Male", "Female"))

# Helper to compute wage
predict_wage <- function(beta3) {
  with(grid,
       beta0 + beta1 * female + beta2 * educ + beta3 * female * educ)
}

grid$wage_a <- predict_wage(beta3_neg)  # Panel (a)
grid$wage_b <- predict_wage(beta3_pos)  # Panel (b)

# -------------------------------------------------------------------
# 3. Plot panels -----------------------------------------------------
pal <- c("Male" = "steelblue", "Female" = "firebrick")

p_a <- ggplot(grid, aes(x = educ, y = wage_a, colour = Gender)) +
  geom_line(size = 1.3) +
...
p_b <- ggplot(grid, aes(x = educ, y = wage_b, colour = Gender)) +
  geom_line(size = 1.3) +
  scale_colour_manual(values = pal) +
  labs(
    x = "Years of education",
...
(a): Both lines rise with education, but the line for women is always below the line for men, and the vertical distance grows as education increases (negative $\beta_1$ and $\beta_3$).

(b): Women start below men at low education levels, yet because $\beta_3$ is positive, women’s slope is steeper; the lines converge and eventually cross, so the gap narrows and reverses at higher education.




These models provide a more nuanced understanding of group-specific effects and are essential when policy implications depend on heterogeneous responses across groups.
