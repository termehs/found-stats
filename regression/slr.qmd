...
$\text{SSE} = \sum_{i=1}^{n} \sum_{i=1}^{n} (y_i - \hat{y}_i)^2 = \sum_{i=1}^{n} (y_i - b_0 - b_1 x_i)^2$
...
To find the values of $b_0$ and $b_1$ that minimize the residual sum of squares (RSS):
...
We compute the first derivatives of RSS with respect to $b_0$ and $b_1$:

$$
\frac{\partial \text{RSS}}{\partial b_0} = -2 \sum_{i=1}^{n} (y_i - b_0 - b_1 x_i)
$$
...
To minimize RSS, set both derivatives equal to zero:
