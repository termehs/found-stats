...
::: {.callout-note title="A Key Property of OLS Residuals"}

When a regression model includes an intercept, the OLS residuals always sum to zero:

$$
\sum_{i=1}^{n} e_i = 0.
$$

This means that the positive and negative residuals balance each other out. In other words, the fitted regression line does not systematically overpredict or underpredict the observed values.

This property follows from the least squares method: OLS chooses the intercept and slope to minimize

$$
SSE = \sum_{i=1}^{n} e_i^2.
$$

One consequence of this minimization is that, with an intercept included, the residuals must sum to zero.
