...
- **If the variances are unknown and cannot be assumed equal**, we cannot assume equal variance, we rely on the so called **Welch’s method**, which is an adaptation of the standard two-sample $t$-test that does **not** assume equal population variances. It is more flexible and robust, especially when sample sizes or variances differ between groups.
