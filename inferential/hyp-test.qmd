...
See @fig-mean-ex2 for an illustration fot he test.
...
ggplot(df_plot, aes(x = x, y = y)) +
  ...
  annotate("segment", x = crit_val + 0.6, xend = crit_val + 0.15, y = 0.06, yend = 0.02,
           arrow = arrow(length = unit(0.18, "cm")), color = "black") +
  annotate("segment", x = -crit_val + 0.7, xend = -crit_val + 0.46, y = -0.06, yend =-0.02, arrow = arrow(length = unit(0.2, "cm")), color = "black") +
  ...
  annotate("text", x = crit_val, y = -0.02, label = sprintf("%.3f", crit_val), color = "black", size = 4) +
  annotate("text", x = -crit_val-0.7, y = 0.1, label = "α/2 = 0.025", size = 4) +
  annotate("text", x = crit_val+0.7, y = 0.09, label = "α/2 = 0.025", size = 4) +

  # Observed value
  geom_point(aes(x = t_obs, y = 0), size = 4) +
...
```
The two-tailed test in Example 22.2 using the $t$-distribution.
:::
